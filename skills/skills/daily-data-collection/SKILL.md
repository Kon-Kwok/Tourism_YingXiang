...
**订单汇总指标（与流量数据一并在此 skill 完成）**：
- total_bookings - 入仓计算总单量
- total_pax - 入仓计算总客人量
- gmv - 入仓计算 GMV
...
开始
  ↓
1. 赤兔KPI报表采集（3个报表）
   - 导出Excel → 等待10秒 → 转换JSON → 生成SQL → 入库
   ↓
2. 飞猪订单列表采集
   - HTTP采集（all-pages）→ 预处理 → 订单明细入库 → 订单汇总入库（`total_bookings`/`total_pax`/`gmv`）
   ↓
3. SYCM流量看板采集
   - HTTP采集 → 转换SQL → 入库
   ↓
...
▶ [3/4] 订单明细入库...
  ✓ 入库完成
▶ [4/4] 订单汇总入库（千牛日度关键表）...
  ✓ total_bookings / total_pax / gmv 入库完成
